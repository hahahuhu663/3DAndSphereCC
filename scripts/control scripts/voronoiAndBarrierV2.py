...
def point_inside_polyhedron(point, faceVertices, vertices):
    for face in faceVertices:
        ...
        vertices_face = np.array([vertices[face[iy]] for iy in range(3)])
...
def savePositions(initials, destinations):
    ...
    for i in range(len(initials)):
        ...
        x_destination = destinations[i][0]
        y_destination = destinations[i][1]
        z_destination = destinations[i][2]
    #Write data
...
def value_contributionV2(vertices, point_samples):
    region_value =[]
    # print(point_samples)
    # for i in range(face):
...
# def density_function(vertices, ori_centroid, point_samples):
    # starttime = time.time()
    ...
    # region_value = value_contributionV2(vertices, point_samples)
...
def findNewCentroid(vertices=[], faceVertices=[], centroid=[]):
# def findNewCentroid(vertices=[], centroid=[]):
    # print(point_samples)
    centroid = density_function(vertices, faceVertices, centroid, point_samples)
    # centroid = density_function(vertices, centroid, point_samples)
...
def calculateVoronoi(container, positions, radius, hasDensityFunction, plotSphere):
    ...
    cellsCentroid = []
    cellsFaceVertices = []
    for voronoiCell in container:
        cellsVertices.append(voronoiCell.vertices())
        cellsCentroid.append(voronoiCell.centroid())
        cellsFaceVertices.append(voronoiCell.face_vertices())

    args = [(cellsVertices[i], cellsFaceVertices[i], cellsCentroid[i]) for i in range(len(container))]
    # args = [(cellsVertices[i], cellsCentroid[i]) for i in range(len(container))]

    # print(args)
    # start = time.time()
    pool = ThreadPool(6)
    destinations = pool.starmap(findNewCentroid, args)
    savePositions(positions, destinations)
    # pool.close()
    # pool.join()
    ...
    #     savePositions(i, positions, centroid)
...
def calculateBarrier(positions, obstacles, radius, goals):
    ...
    goalsT = np.array(goals).T
    dxi = si_position_controller(positionForCbf[:3][:], goalsT[:3][:])
...
def calculateVoronoiAndBarrier(container, positions, obstacles=None, radius = 0, hasDensityFunction=False, plotSphere=False):
    ...
    goals = calculateVoronoi(container, positions, radius, hasDensityFunction, plotSphere)

    # print(goals_x, goals_y, goals_z)
    # return calculateBarrier(positions, obstacles, radius, goals_x, goals_y, goals_z)
    return calculateBarrier(positions, obstacles, radius, goals)

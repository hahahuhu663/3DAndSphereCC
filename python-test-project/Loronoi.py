...
def circumcircle( pointlist , normdir=-1 ):
    ...
    if N != 3:
        ...
        p1 = convertToVector(pointlist[0])
        p2 = convertToVector(pointlist[1])
        p3 = convertToVector(pointlist[2])
        if normdir == -1:
...
######## check and see if all points are unique.  if not, raise an error.
        for i in range(len(self.points)):
            for j in range(i+1, len(self.points)):
                array = self.points[i] - self.points[j]

                dist2 = mag2( vector(array[0], array[1], array[2]))

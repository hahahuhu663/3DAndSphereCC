...
def calculateVoronoiAndBarrier(positions, obstacles=None, radius = 0, hasDensityFunction=False, plotSphere=False):
    # goals_x, goals_y, goals_z = calculateVoronoi(container, positions, radius, hasDensityFunction, plotSphere)
    goals_x, goals_y, goals_z = [goal[0]], [goal[1]], [goal[2]]
...
currentPos = drone[0]
i=1
while calGoalDist(currentPos[0],currentPos[1],currentPos[2])>=1:
# for i in range(300):
    # c = createVoronoiContainer(drone, voronoiLimit, False)
    dxi = calculateVoronoiAndBarrier(drone, obstacles, 8.7, False, True)
    drone[0][0] = drone[0][0] + dxi[0][0] 
    drone[0][1] = drone[0][1] + dxi[1][0]
    drone[0][2] = drone[0][2] + dxi[2][0]
    listDataX.append(drone[0][0])
    listDataY.append(drone[0][1])
    listDataZ.append(drone[0][2])
    currentPos=[drone[0][0],drone[0][1],drone[0][2]]
    print(i)
    i+=1
ax.plot(listDataX,listDataY, listDataZ, c='r', label='drone1')
plt.legend()
plt.savefig('/home/robolab/python-test-project/trajectoriesShowcase'+'SuperEllipsoid'+'.png')
# plt.savefig('/home/robolab/python-test-project/trajectoriesShowcase'+'Sphere'+'.png')

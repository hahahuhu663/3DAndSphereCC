...
error = pd.read_csv(folder_name+'/dronesShortestObstacles.csv', header = None)
...
for i in range(1,9):
    objectNumber = i
    plt.plot(error[16], error[objectNumber*2 -2], label='Minimum Distance')
    # plt.plot(error[3], error[1], label='Minimum Distance')

    # plt.axhline(y = 8.7, color = 'r', linestyle = '--', label='Safety Radius')
    plt.plot(error[16], error[objectNumber*2 -1], color = 'r', linestyle = '--', label='Safety Radius')
    ...
    plt.savefig('/home/robolab/plot_result/'+ folder_name +'/error_shortest_cbf_drone1_'+ str(objectNumber) +'.png')
    plt.close()
